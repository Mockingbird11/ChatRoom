...
namespace ChatRoom.DataAcces.Redis
{
    public class RedisHelper
    {
        ...
        public RedisHelper()
        {
            //redis缓存
            //连接字符串
            string connectionString = ConfigHelper.GetSectionValue("Redis:Connection");
            //实例名称
            string instanceName = ConfigHelper.GetSectionValue("Redis:InstanceName");
            //默认数据库
            int defaultDB = int.Parse(ConfigHelper.GetSectionValue("Redis:DefaultDB") ?? "0");
        ...
        {
            return Do(db => db.StringSet(keyValues.ToArray()));
        }
        ...
        {
            return await Do(db => db.StringSetAsync(keyValues.ToArray()));
        }
        ...
        {
            return Do<List<string>>(db => this.ConvertString<string>(db.HashKeys(key, CommandFlags.None)));
        }
        ...
        {
            return GetConnect().GetServer(hostAndPort);
        }
